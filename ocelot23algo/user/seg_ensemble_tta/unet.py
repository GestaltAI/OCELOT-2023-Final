...
class PytorchUnetCellModel():
    ...
    def __init__(self, metadata):
        self.device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu') #cuda:0
    ...
    def load_checkpoint(self):
        ...
        _path_to_checkpoint = os.path.join(_curr_path, "checkpoints/baseline_unet/ocelot_unet.pth")
    ...
    def prepare_input(self, cell_patch):
        ...
        -------
            ...
            dimension
        """
        cell_patch = torch.from_numpy(cell_patch.copy()).permute((2, 0, 1)).unsqueeze(0)
        cell_patch = cell_patch.to(self.device).type(torch.FloatTensor) #.type(torch.cuda.FloatTensor)

        if torch.cuda.is_available():
            cell_patch = cell_patch.type(torch.cuda.FloatTensor)
    ...
    def __call__(self, cell_patches, tissue_patchs, pair_id):
        ...
        -------
            List[tuple]: for each predicted cell we provide the tuple (x, y, cls, score)
        """
        cell_patches = [self.prepare_input(cell_patch) for cell_patch in cell_patches] # cv2.cvtColor(cell_patch, cv2.COLOR_BGR2RGB)
        logits = [self.unet(cell_patch) for cell_patch in cell_patches]
        logits[1] = torch.flip(logits[1], dims=(2,))
        logits[2] = torch.flip(logits[2], dims=(3,))
        logits[3] = torch.flip(logits[3], dims=(2, 3))
        heatmap = [self.post_process(logit) for logit in logits]
        return self.find_cells(torch.mean(torch.cat(heatmap), dim=0, keepdim=True))
